...
import { MATCH_LABELS, netMarginInfo } from '../../../lib/scoring'
...
export function Results({ data }) {
  ...
  data.slots.forEach((slot) => {
    if (slot.abandonment) return // discarded — see computePointsTable
    slot.matches.forEach((m) => { if (m.result) rows.push({ slot, m }) })
  })
    ...
    <div>
      ...
      <div className="space-y-2">
        {rows.map(({ slot, m }) => {
          ...
          const info = netMarginInfo(m.result)
          const bonusNote = m.result.bonusRunsApplied
            ? `${m.result.bonusRunsApplied > 0 ? '+' : ''}${m.result.bonusRunsApplied} bonus runs (${teamById(m.result.advantageTeamId)?.name}'s advantage)`
            : null
          return (
            <Card key={m.id}>
              <div className="flex items-center justify-between">
                ...
                {info && !info.full && <span className="tag-neutral text-[9px] uppercase tracking-wide px-1.5 py-0.5 rounded">Shortened</span>}
              </div>
              <div className="flex items-center justify-between mt-1.5">
                ...
                <div className="text-right text-xs" style={{ color: 'var(--muted)' }}>
                  ...
                  {m.result.walkover && <p>without batting — opponent's net total went negative</p>}
              ...
              {bonusNote && <p className="text-[10px] mt-1.5" style={{ color: 'var(--gold)' }}>{bonusNote}</p>}
            </Card>
          )
        })}
      </div>
